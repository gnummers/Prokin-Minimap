...
- Keeps a PvP or battleground proxy button outside the square minimap border only when Blizzard battlefield state is active, with left-click and right-click queue menu access while queued through Blizzard's native secure menu flow
...
Left-drag the visible Blizzard minimap widgets to reorganize them around the square border. Their positions are saved in `ProkinMinimapDB`, the tracking proxy still opens Blizzard's tracking menu when clicked, and the battlefield proxy routes queued menu actions through Blizzard's native battlefield button so queue actions stay protected.
